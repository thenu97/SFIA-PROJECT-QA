...
def test_about():
    http = urllib3.PoolManager()
    r = http.request('GET', 'http://localhost:5000/about')
    assert 200 == r.status

def test_create():
    http = urllib3.PoolManager()
    r = http.request('GET', 'http://localhost:5000/create')
    assert 200 == r.status

def test_addtag():
    http = urllib3.PoolManager()
    r = http.request('GET', 'http://localhost:5000/tag')
    assert 200 == r.status

def test_view():
    http = urllib3.PoolManager()
    r = http.request('GET', 'http://localhost:5000/blog/2')
    assert 200 == r.status
